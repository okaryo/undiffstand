...
describe("DiffSelector", () => {
  it("shows the current branch name for the default review", async () => {
    ...
    ).not.toHaveTextContent("HEAD");
    expect(screen.getByRole("combobox", { name: "To" })).toHaveTextContent(
      "Working tree",
    );
  ...
  it("shows capped recent lists and expands local and remote branches on demand", async () => {
    ...
    const recentBranches = within(menu).getByRole("region", {
      name: "Recent branches",
    });
    const recentCommits = within(menu).getByRole("region", {
      name: "Recent commits",
    });
    ...
    expect(
      within(menu).queryByRole("option", { name: "main" }),
    ).not.toBeInTheDocument();
    expect(
      within(menu).queryByRole("option", { name: "origin/release" }),
    ).not.toBeInTheDocument();

    await fireEvent.click(
      within(menu).getByRole("button", { name: /Local branches/ }),
    );
    expect(
      within(menu).getByRole("option", { name: "main" }),
    ).toBeInTheDocument();
    await fireEvent.click(
      within(menu).getByRole("button", { name: /Remote branches/ }),
    );
    expect(
      within(menu).getByRole("option", { name: "origin/release" }),
    ).toBeInTheDocument();
  });
