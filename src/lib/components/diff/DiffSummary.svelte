...
<div
  class="summary"
  aria-label={`${filesLabel}, ${additionsLabel}, ${deletionsLabel}`}
>
  <strong class="files" title={filesLabel}>{summary.files.length}</strong>
  <strong class="addition" title={additionsLabel}
    >+{summary.totalAdditions}</strong
  >
  <strong class="deletion" title={deletionsLabel}
    >−{summary.totalDeletions}</strong
  >
</div>
