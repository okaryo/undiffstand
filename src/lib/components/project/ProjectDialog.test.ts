...
describe('ProjectDialog', () => {
  ...
  it('requires a base branch when detection fails', async () => {
    ...
    expect(
      screen.getByText('Base branch could not be detected. Select it to continue.')
    ).toBeInTheDocument();

    await fireEvent.click(screen.getByRole('combobox', { name: 'Base branch' }));
    await fireEvent.click(
      within(screen.getByRole('listbox', { name: 'Base branch options' })).getByRole('option', {
        name: 'develop'
      })
    );
    expect(saveButton).toBeEnabled();
    expect(
      screen.queryByText('Base branch could not be detected. Select it to continue.')
    ).not.toBeInTheDocument();

    await fireEvent.click(saveButton);
    expect(onSave).toHaveBeenCalledWith(expect.objectContaining({ baseRef: 'develop' }));
  });
});
