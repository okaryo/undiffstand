...
export const nativeApi: AppApi = {
  ...
  saveUserPreferences: (preferences) => invoke('save_user_preferences', { preferences }),
  getDiffSummary: (projectId, selection) => invoke('get_diff_summary', { projectId, selection }),
  ...
  runChangeReview: (projectId, selection) => invoke('run_change_review', { projectId, selection })
};
