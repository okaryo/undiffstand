...
describe("diff display labels", () => {
  it("shows the current branch name instead of HEAD", () => {
    expect(revisionDisplayLabel("HEAD", "feature/undiffstand")).toBe(
      "feature/undiffstand",
    );
    expect(
      diffSelectionLabel({ base: "HEAD", target: "." }, "feature/undiffstand"),
    ).toBe("feature/undiffstand → working tree");
    expect(
      diffComparisonLabel(
        { fromLabel: "HEAD", toLabel: "main" },
        "feature/undiffstand",
      ),
    ).toBe("feature/undiffstand → main");
  });
